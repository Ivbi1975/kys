...
export function SummaryReportOverlay({
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto shadow-2xl flex flex-col"
      ...
      >
        ...
        <div className="p-6 space-y-6 rapor-summary-print">
          {/* Title block */}
          <div className="text-center">
            <h3 className="text-lg font-extrabold" style={{ color: "#f8fafc" }}>{data.kesimAlaniName}</h3>
            {data.projectName && (
              <p className="text-sm mt-0.5" style={{ color: "#94a3b8" }}>{data.projectName}</p>
            )}
            <div className="flex items-center justify-center gap-1.5 mt-1.5">
              <Calendar className="w-3.5 h-3.5" style={{ color: "#94a3b8" }} aria-hidden="true" />
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="relative shrink-0" style={{ width: 120, height: 120 }}>
                <svg width="120" height="120" viewBox="0 0 120 120" className="-rotate-90" aria-hidden="true">
                  <circle cx="60" cy="60" r="52" fill="none" strokeWidth="10" stroke="rgba(148,163,184,0.10)" />
                  <circle
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-3xl font-black tabular-nums leading-none" style={{ color: progressColor }}>
                    %{percentage}
                  </span>
                  <span className="text-[10px] font-semibold mt-1" style={{ color: "#94a3b8" }}>tamamlandı</span>
                </div>
              </div>
              ...
              <div className="flex-1 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs" style={{ color: "#94a3b8" }}>Toplam</span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs" style={{ color: "#94a3b8" }}>Kesildi</span>
                  </div>
                  <span className="text-xl font-black tabular-nums" style={{ color: "#00c986" }}>{completed}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ background: "#f59e0b" }} />
                    <span className="text-xs" style={{ color: "#94a3b8" }}>Bekleyen</span>
                  </div>
                  <span className="text-xl font-black tabular-nums" style={{ color: remaining > 0 ? "#f59e0b" : "#94a3b8" }}>{remaining}</span>
                </div>

                {/* mini progress bar */}
                <div className="pt-1">
                  <div className="w-full rounded-full overflow-hidden" style={{ height: 6, background: "rgba(148,163,184,0.12)" }}>
                    <div
                      className="h-full rounded-full transition-all duration-700"
                      style={{ width: `${Math.max(percentage, completed > 0 ? 1 : 0)}%`, background: progressColor }}
                    />
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            >
              ...
              <p className="text-[11px] font-semibold mt-1" style={{ color: "#94a3b8" }}>Toplam</p>
            </div>
            <div
              className="rounded-xl p-4 text-center"
              style={{ background: "rgba(0,201,134,0.08)", border: "1px solid rgba(0,201,134,0.15)" }}
            >
              <p className="text-2xl font-black tabular-nums" style={{ color: "#00c986" }}>{completed}</p>
              <div className="flex items-center justify-center gap-1 mt-1">
                <CheckCircle2 className="w-3 h-3" style={{ color: "#00c986" }} aria-hidden="true" />
                <p className="text-[11px] font-semibold" style={{ color: "#00c986" }}>Kesildi</p>
              </div>
            </div>
            <div
              className="rounded-xl p-4 text-center"
              style={{
                background: remaining > 0 ? "rgba(245,158,11,0.08)" : "rgba(148,163,184,0.06)",
                border: remaining > 0 ? "1px solid rgba(245,158,11,0.15)" : "1px solid rgba(148,163,184,0.10)"
              }}
            >
              <p className="text-2xl font-black tabular-nums" style={{ color: remaining > 0 ? "#f59e0b" : "#94a3b8" }}>{remaining}</p>
              <p className="text-[11px] font-semibold mt-1" style={{ color: remaining > 0 ? "#f59e0b" : "#94a3b8" }}>Kalan</p>
            </div>
          </div>
          ...
          {(firstKesildi || lastKesildi) && (
            <div
              ...
              style={{ background: "rgba(148,163,184,0.05)", border: "1px solid rgba(148,163,184,0.10)" }}
            >
              <div className="flex items-center gap-2">
                <Clock className="w-3.5 h-3.5" style={{ color: "#94a3b8" }} aria-hidden="true" />
                <h4 className="text-xs font-bold uppercase tracking-wider" style={{ color: "#94a3b8" }}>Zaman Bilgisi</h4>
              </div>
              {firstKesildi && (
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: "#60a5fa" }} />
                    <span className="text-xs" style={{ color: "#94a3b8" }}>İlk kesim</span>
                  </div>
              ...
              {lastKesildi && lastKesildi.animalNo !== firstKesildi?.animalNo && (
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs" style={{ color: "#94a3b8" }}>Son kesim</span>
                  </div>
                  <span className="text-xs font-semibold text-right" style={{ color: "#cbd5e1" }}>
          ...
          {teamStats.length > 0 && (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <Users className="w-3.5 h-3.5" style={{ color: "#94a3b8" }} aria-hidden="true" />
                <h4 className="text-xs font-bold uppercase tracking-wider" style={{ color: "#94a3b8" }}>Ekip Durumu</h4>
              </div>
                    ...
                    <div key={t.name}>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <div className="flex items-center gap-3 text-xs">
                          <span style={{ color: "#94a3b8" }}>{t.completed}/{t.total}</span>
                          <span
                            className="font-bold tabular-nums min-w-[36px] text-right"
                            style={{ color: pct === 100 ? "#00c986" : "#f8fafc" }}
                          >
                            %{pct}
                          </span>
                        </div>
                      ...
                      <div className="w-full rounded-full overflow-hidden" style={{ height: 6, background: "rgba(148,163,184,0.12)" }}>
                        <div
                          className="h-full rounded-full transition-all duration-700"
                          style={{ width: `${Math.max(pct, t.completed > 0 ? 2 : 0)}%`, backgroundColor: t.color }}
                        />
                      </div>
                    </div>
                  );
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <StickyNote className="w-3.5 h-3.5" style={{ color: "#94a3b8" }} aria-hidden="true" />
              <h4 className="text-xs font-bold uppercase tracking-wider" style={{ color: "#94a3b8" }}>Notlar ve Talepler</h4>
            </div>
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Edit3 className="w-3.5 h-3.5" style={{ color: "#94a3b8" }} aria-hidden="true" />
              ...
              {pendingReqCount > 0 && (
                ...
                >
                  <div className="flex items-center gap-2">
                    <AlertCircle className="w-3.5 h-3.5" style={{ color: "#f59e0b" }} aria-hidden="true" />
                    <span className="text-sm font-semibold" style={{ color: "#f59e0b" }}>Bekleyen talepler</span>
                  </div>
                  <span
                    className="text-sm font-bold tabular-nums px-2.5 py-0.5 rounded-lg"
          ...
          {/* Footer timestamp */}
          <p className="text-[11px] text-center pb-1" style={{ color: "#475569" }}>
