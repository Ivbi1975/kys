...
import { BATCH_SIZE, LARGE_BATCH_SIZE, ERROR_MESSAGES, NOTE_WARNING_KEYWORDS } from "../../lib/constants";
import { getFullKesimAlani } from "../../services/kesim-alani.service";
    ...
    if (affectedGroupIds.length > 0) {
      ...
      for (const row of groupMembersResult.rows as any[]) {
        if (!donationById[row.donation_id]) {
          donationById[row.donation_id] = row as ConflictDonationRow;
        }
